...
# 分析相同tag觀看次數差異，以culture為例
ted_culture <- ted_main %>%
  mutate(tag_list = tags %>%
           strsplit(',') %>% 
           str_match_all('[A-Z|a-z]{2,}')) %>%
  select(tag_list, url, duration, views, tags) %>%
  unnest(tag_list) %>%
  filter(tag_list == "culture") %>%
  arrange(desc(views)) %>%
  mutate(tag_seg = tags %>%
           strsplit(',') %>% 
           str_match_all('[A-Z|a-z]{2,}'))
...
# 計算前後30個影片長度、tags數量
cul_top30 <- head(ted_culture, 30)
num_tags_t <- vector('numeric', 30)
for (i in 1:30) {
  num_tags_t[i] <- length(cul_top30$tag_seg[[i]])
}

cul_top30_df <- data.frame(duration = cul_top30$duration,
                           num_tags = num_tags_t,
                           rank = "top")

cul_last30 <- tail(ted_culture, 30)
num_tags_l <- vector('numeric', 30)
for (i in 1:30) {
  num_tags_l[i] <- length(cul_last30$tag_seg[[i]])
}

cul_last30_df <- data.frame(duration = cul_last30$duration,
                            num_tags = num_tags_l,
                            rank = "last")

cul_df <- rbind(cul_top30_df, cul_last30_df)

# culture tag數量機率分布
cul_df %>% 
  ggplot() +
  geom_density(aes(num_tags, color = rank)) +
  labs(title = 'culture相關影片tag數量分布') +
  xlab('次數') +
  theme_light()
...
cul_df %>% 
  ggplot() +
  geom_density(aes(duration, color = rank)) +
  labs(title = 'culture相關影片長度分布') +
  xlab('長度（秒）') +
  theme_light()

# 篩選出culture前後30的逐字稿
scripts_cul_top30 <- filter(scripts_all, url %in% cul_top30$url)
scripts_cul_last30 <- filter(scripts_all, url %in% cul_last30$url)

# 製作詞頻表
scripts_cul_top30 <- scripts_cul_top30 %>%
  select(transcript) %>%
  unnest_tokens(output = "word", input = "transcript",
                token = "regex", pattern = "[ ,:;\\.\"—)]") %>%
  anti_join(stop_words, by = c("word" = "word")) %>% 
...
scripts_cul_last30 <- scripts_cul_last30 %>%
  select(transcript) %>%
  unnest_tokens(output = "word", input = "transcript",
                token = "regex", pattern = "[ ,:;\\.\"—)]") %>%
  anti_join(stop_words, by = c("word" = "word")) %>% 
